...
ENT.PrintName		= "Shield Generator (Blue) (Invincible)"
...
function ENT:SetupDataTables()
	self:NetworkVar("Bool",0,"Enable", { KeyName = "shbienabled", Edit = { type = "Boolean", order = 1, min = 0, max = 1 } } )
	self:NetworkVar("Float",0,"ShieldSize", { KeyName = "shbiradius", Edit = { type = "Float", order = 3, min = 5, max = 200 } } )
	self:NetworkVar("Bool",1,"BlockerExists")
	self:NetworkVar("Bool",2,"IsRecharging")
	self:NetworkVar("Bool",3,"IsUseable", { KeyName = "shbiuseable", Edit = { type = "Boolean", order = 2, min = 0, max = 1 } } )
...
function ENT:CreateBlocker()
	...
	self.Blocker.IsInvincibleShield = true
end
